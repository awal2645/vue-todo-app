<template>
<div class="h-screen w-screen bg-gradient-to-tr from-blue-400 to-red-300 flex flex-col items-center justify-center px-4 space-y-3">
    <!-- Logo -->
    ...
    <div class="bg-white p-8 w-full max-w-xl rounded">
        ...
        <form @submit.prevent="addTask()" class="flex">
            <input type="text" v-model="taskInput" class="block w-full border border-gray-300 focus:border-gray-400 focus:outline-none rounded-l-md px-4 py-1.5" placeholder="Add task to your list" />
            <button type="submit" class="border border-gray-300 border-l-0 rounded-r-md px-6 py-1 bg-indigo-500 hover:bg-indigo-600 text-white">
                Add
            </button>
        </form>
        <!-- Add task end-->
        ...
        <div class="flex items-center justify-between mt-4">
            ...
            <div class="flex items-center h-8">
                ...
                <div v-show="isSearch" class="relative">
                    <input v-model="searchInput" type="text" class="w-56 rounded-md block border border-gray-300 text-sm px-7 py-1" placeholder="search your task" />
                    <span class="absolute left-1.5 top-2 text-gray-400">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 " fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                    </span>
                    <button @click="isSearch=false" class="absolute right-1.5 top-2 text-gray-400 hover:text-gray-700">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Clear all -->
        <div class="mt-4">
            <button @click="clear()" class="px-6 py-1.5 text-white bg-indigo-500 hover:bg-indigo-600 rounded text-sm">
                Clear all
            </button>
        </div>
        <!-- Clear all end -->
